'use strict';
module.exports = function createRateLimitChecker(CORSESCAPE_RATELIMIT) {
  // Configure rate limit. The following format is accepted for CORSESCAPE_RATELIMIT:
  ...
  var rateLimitConfig = /^(\d+) (\d+)(?:\s*$|\s+(.+)$)/.exec(CORSESCAPE_RATELIMIT);
  if (!rateLimitConfig) {
    ...
    unlimitedPattern.trim().split(/\s+/).forEach(function(unlimitedHost, i) {
      ...
      if (startsWithSlash || endsWithSlash) {
        if (unlimitedHost.length === 1 || !startsWithSlash || !endsWithSlash) {
          throw new Error('Invalid CORSESCAPE_RATELIMIT. Regex at index ' + i +
  ...
  var rateLimitMessage = 'The number of requests is limited to ' + maxRequestsPerPeriod +
    ...
    'Please self-host CORS Escape if you need more quota. ' +
