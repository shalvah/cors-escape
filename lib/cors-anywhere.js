...
function showUsage(helpFile, headers, response) {
  const isHtml = /\.html$/.test(helpFile);
  headers['content-type'] = isHtml ? 'text/html' : 'text/plain';
  if (helpText[helpFile] != null) {
    response.writeHead(200, headers);
    response.end(helpText[helpFile]);
  } else {
    require('fs').readFile(helpFile, 'utf8', function(err, data) {
      if (err) {
        console.error(err);
        ...
        showUsage(helpFile, headers, response); // Recursive call, but since data is a string, the recursion will end
      }
    });
  }
}
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    const location = parseURL(req.query.url || req.url.slice(1));

    if (!location) {
      // Invalid API call. Show how to correctly use the API
      showUsage(corsAnywhere.helpFile, corsHeaders, res);
      return;
    }
